...
def extract_performance(y, indicator):
    ...
    for i in range(9, len(y)):
        p = is_pareto_efficient(y[:i], return_mask=True)
        ans = indicator.calc(y[:i][p])
...
lhs_ind = int(np.where([np.shape(result["igd+"]) == (0,) for result in results])[0]) # find which result is lhs

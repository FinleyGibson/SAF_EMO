...
def get_missing(a, total, targets=None): 
    b = np.arange(total) 
    duplicates = a.copy() 
    missing = [] 
    for i in b: 
...
for directory in results_dirs:
    target = get_target_from_str(directory)
    ...
    if sorted(seeds) == sorted(expected):
        ...
        duplicates, missing = get_missing(seeds, 31, targets)
        if len(duplicates)>0:
    ...
    print('\n')
    D.append({'directory': directory, 'duplicates': duplicates, 'missing': missing, 'seeds':seeds, 'target': target})
